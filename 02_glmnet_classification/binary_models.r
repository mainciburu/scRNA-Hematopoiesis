...
ncores<-args[7]         # number of cores for parallelization
...
x<-seurat.train@assays$integrated@var.features
...
seurat.train@meta.data[,celltype][seurat.train$CellType2 == celltype]<-1

############ START LOOP ################################
# 10 parallel iterations
# Store the resulting prediction probabilities from each iteration
#for(z in 1:r){
registerDoParallel(cores=ncores)
pred.tmp<-foreach(z = 1:r, .combine = "cbind", .packages = c("Seurat", "glmnet",
                                                             "ROCR", "pROC", "caret"))%dopar%{
  ...
  best<-rownames(best)
  print(paste0("    Round ", z, " Optimal alpha: ", best))

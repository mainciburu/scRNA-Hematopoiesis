...
# MDS
mds<-readRDS("/home/mainciburu/scRNA/MDS_paper/seurat_mds10.rds")

g<-g[g%in%rownames(mds)]

x<-mds@assays$RNA@data[g,]
write.csv(x = x, file = "/home/mainciburu/scRNA/scenic/data/mds1_norm_mat_full.csv")
MetaD<-data.frame(CellType=mds$prediction)
write.table(MetaD,file="/home/mainciburu/scRNA/scenic/data/mds10_CellType.txt",col.names =T,row.names = T)
